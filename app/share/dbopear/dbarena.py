...
'''def insertMatchQuene(characterId,score):
    #datestr = datetime.date.today()
    sql = "insert into tb_matchQuene(characterId,score)\
     values(%d,'%d')"%(characterId,score)
    conn = dbpool.connection()
    cursor = conn.cursor()
    cursor.execute(sql)
    conn.commit()
...
def updateCharacterArenaScore(characterId,change,oriscore):
    '''更新角色的天梯分
    '''
    sqlstr = "update tb_arena set score = %d where characterId =%d"%((oriscore + change),characterId)
    conn = dbpool.connection()
    cursor = conn.cursor()
    count = cursor.execute(sqlstr)
    conn.commit()
...
def getCharacterScoreRank(characterId):
    '''获取角色的排名
    '''
    sql = "SELECT score,ranking from tb_arena where characterId = %d;"%(characterId)
    conn = dbpool.connection()
    cursor = conn.cursor(cursorclass=DictCursor)
    cursor.execute(sql)
    result = cursor.fetchone()
...
def match(selfscore):
    scoremax = selfscore + 200
    scoremin = selfscore - 200
    sql = "SELECT characterId , score from \
        tb_arena as b where  b.ifmatch = '%d'\
        and b.score > '%d' and b.score < '%d' and b.score <> '%d'"% (1, scoremin, scoremax,selfscore)
    conn = dbpool.connection()
    cursor = conn.cursor(cursorclass=DictCursor)
    # 执行SQL语句
    cursor.execute(sql)
...
def getCharacterRivalList(ranklist):
    ...
    if orsql:
        sql = "SELECT b.characterId,b.score,\
        a.nickname,a.level,a.sex from tb_character as a,\
