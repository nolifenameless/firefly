...
@remoteserviceHandle
def GetfubenInfo_3333(dynamicId,request_proto):
    argument = json.loads(request_proto)
    characterId = argument.get('characterId')
    data = zhanyi.getfubenInfo(dynamicId, characterId)
    return json.dumps(data)

@remoteserviceHandle
def ClientAdd_3334(dynamicId,request_proto):
    argument = json.loads(request_proto)
    print argument
    characterId = argument.get('characterId')
    data = zhanyi.setfubenInfo(dynamicId, characterId,argument)
    return json.dumps(data)