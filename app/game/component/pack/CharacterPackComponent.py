...
#        itemsPackInfo = db_package.getAllItemsByCharacterId(self._owner.baseInfo.id)
    ...
    def setEquipmentSlot(self,size = 9):
        '''设置角色装备栏
        @param size: int 装备栏默认为9个部位
        '''
    ...
    def equipEquipmentByItemId(self,itemId):
        ...
        item = self._package.getItemById(itemId)
        if not item:
            return {'result':False,'message':u"11"}
        ...
        if toPosition<0 or toPosition>8:
        ...
        if not toItemid or toItemid<0:
            ...
            if result2:
                ...
                return {'result':False,'message':u"5"}
        else:
            result1 = equipPackage.updateEquipment(toPosition, itemId)
            if result1:
                return {'result':True,'message':""}
            else:
                return {'result':False,'message':u"6"}

    def equipEquipmentByItemIdPos(self,itemId,pos):
        '''穿上
        @param fromPosition: int 物品在包裹中的位置
        @param toPosition: int 装备的位置
        '''
    ...
    def getStoneNum(self,characterId):
        '''获取包裹的强化石数量
        '''
        data = {}
        itemList = self._package._items.values()
        data['stone'] = [{'itemid':itemInfo.baseInfo.id,
                             'stack':itemInfo.pack.getStack(),
                             #'stack':dbItems.getItem(characterId,60001)[0]['stack']
                         }\
                             for itemInfo in itemList \
                             if itemInfo.baseInfo.getItemTemplateId() == 6001]
        return {'result':True,'stone':data}
    ...
    def getSkillList(self):
        ...
        data['itemlist'] = [{'itemid':itemInfo.baseInfo.id,
                             ...
                             for itemInfo in itemList \
                             if itemInfo.baseInfo.getItemTemplateId() > 1000 and itemInfo.baseInfo.getItemTemplateId() < 2000]
        data.update({'Num':len(data['itemlist'])})
        return {'result':True,'data':data}

    def getFashionList(self):
        '''获取包裹的时装信息
        '''
        data = {}
        i = 0;
        ...
        data['itemlist'] = [{'itemid':itemInfo.baseInfo.id,
                             ...
                             for itemInfo in itemList \
                             if itemInfo.baseInfo.getItemTemplateId() > 3000 and itemInfo.baseInfo.getItemTemplateId() < 4000]
        data.update({'Num':len(data['itemlist'])})
        return {'result':True,'data':data}
    ...
    def qianghua(self,itemId,level,characterId):
        '''强化
        '''
        result = dbItems.setItem(level,characterId,itemId)
        dbTask.setDailyTaskaInfo(characterId,100010,1)
        return result
