...
#        return{'result':False}
    ...
    return{'result':True,'message':u'login server ','data':UserCharacterInfo}
    
def activeNewPlayer(dynamicId,userId,nickName,sex):
    '''创建角色
    ...
    result = user.creatNewCharacter(nickName, sex)
    return result
...
def enterScene(dynamicId, characterId, placeId,force):
    ...
    d = GlobalObject().root.callChild(nownode,601,dynamicId, characterId,None)
